package com.WazCodes.narcoticsPlugin;

import com.WazCodes.narcoticsPlugin.events.NarcoticConfigReloadEvent;
import com.WazCodes.narcoticsPlugin.handlers.ConfigHandler;
import com.WazCodes.narcoticsPlugin.handlers.DrugHandler;
import com.WazCodes.narcoticsPlugin.handlers.EventsHandler;
...
public class Narcotic extends JavaPlugin {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
        if (cmd.getName().equalsIgnoreCase("narcotic") || cmd.getName().equalsIgnoreCase("narc")) {
            if (args.length == 1) {
                if (args[0].equalsIgnoreCase("reload")) {
                    if (sender.hasPermission("drugs.reload")) {
                        ...
                        getServer().getPluginManager().registerEvents(new EventsHandler(this), this);
                        sender.sendMessage(ChatColor.GREEN + "[Narcotics] Reloaded!");
                        Bukkit.getServer().getPluginManager().callEvent(new NarcoticConfigReloadEvent());
                        return true;
                    }
                }
                ...
                if (args[0].equalsIgnoreCase("cleardrugs") ||
                    ...
                    if (sender.hasPermission("drugs.cleardrugs")) {
                        ...
                        if (p == null) {
                            sender.sendMessage(ChatColor.RED + "[Narcotic] '" + args[1] + "' is not online.");
                            return true;
                        } else {
                            if (getPlayerHandler().getOnDrugs().containsKey(p.getName())) {
                                ...
                                p.sendMessage(ChatColor.GREEN + "[Narcotic] All of your drug effects have been "
                                        + "cleared!");
                                sender.sendMessage(ChatColor.GREEN + "[Narcotic] Cleared drug effects from '" + p
                                        .getName() + "'.");
                                return true;
                            }
                        }
